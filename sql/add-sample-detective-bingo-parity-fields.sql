...
ALTER TABLE public.sd_sessions
...
ALTER TABLE public.sd_sessions
  DROP CONSTRAINT IF EXISTS sd_sessions_default_intermission_seconds_chk;

ALTER TABLE public.sd_sessions
  ADD CONSTRAINT sd_sessions_default_intermission_seconds_chk CHECK (default_intermission_seconds >= 0);

UPDATE public.sd_sessions
SET
  welcome_heading_text = COALESCE(NULLIF(welcome_heading_text, ''), 'Welcome to Sample Detective'),
  welcome_message_text = COALESCE(NULLIF(welcome_message_text, ''), 'Match each sampled track to its original source.'),
