...
ALTER TABLE public.aa_sessions
  DROP CONSTRAINT IF EXISTS aa_sessions_default_intermission_seconds_chk;

ALTER TABLE public.aa_sessions
  ADD CONSTRAINT aa_sessions_default_intermission_seconds_chk CHECK (default_intermission_seconds >= 0);

UPDATE public.aa_sessions
SET
  welcome_heading_text = COALESCE(NULLIF(welcome_heading_text, ''), 'Welcome to Artist Alias'),
  welcome_message_text = COALESCE(NULLIF(welcome_message_text, ''), 'Name the artist from the clues revealed one stage at a time.'),
