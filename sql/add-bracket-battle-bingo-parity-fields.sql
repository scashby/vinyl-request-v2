...
ALTER TABLE public.bb_sessions
  DROP CONSTRAINT IF EXISTS bb_sessions_default_intermission_seconds_chk;

ALTER TABLE public.bb_sessions
  ADD CONSTRAINT bb_sessions_default_intermission_seconds_chk CHECK (default_intermission_seconds >= 0);

UPDATE public.bb_sessions
SET
  welcome_heading_text = COALESCE(NULLIF(welcome_heading_text, ''), 'Welcome to Bracket Battle'),
  welcome_message_text = COALESCE(NULLIF(welcome_message_text, ''), 'Vote for your favourite track in each matchup to advance seeds through the bracket.'),
