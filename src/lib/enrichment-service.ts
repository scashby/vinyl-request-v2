...
export async function enrichMusicBrainz(albumId: number, artist: string, title: string): Promise<{ success: boolean; error?: string; skipped?: boolean }> {
  try {
    ...
    const { error } = await supabase.from('v2_legacy_archive').update(updateData).eq('id', albumId);
...
export async function enrichDiscogsTracklist(albumId: number): Promise<{ success: boolean; error?: string; skipped?: boolean }> {
  try {
    const { data: album } = await supabase.from('v2_legacy_archive').select('discogs_release_id, tracks, artist').eq('id', albumId).single();
    ...
    const { error } = await supabase.from('v2_legacy_archive').update({ tracks: updatedTracks }).eq('id', albumId);
    if (error) return { success: false, error: error.message };
    return { success: true };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
...
export async function enrichCoverArtArchive(albumId: number): Promise<{ success: boolean; error?: string; skipped?: boolean }> {
    try {
        // 1. Get MBID from DB
        const { data: album } = await supabase.from('v2_legacy_archive').select('musicbrainz_id').eq('id', albumId).single();
