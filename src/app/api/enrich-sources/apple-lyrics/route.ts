...
export async function POST(req: Request) {
  ...
  try {
    ...
    const bodyAppleAlbumId = typeof body?.appleMusicAlbumId === 'string' ? body.appleMusicAlbumId : null;
    const creditsAlbumId = (release?.release_tracks ?? [])
      .map((track) => toSingle(track.recording)?.credits)
      .find((credit) => typeof credit === 'object' && credit && !Array.isArray(credit) && typeof (credit as Record<string, unknown>).apple_music_album_id === 'string') as Record<string, unknown> | undefined;
    const appleAlbumId =
      (creditsAlbumId?.apple_music_album_id as string | undefined) ??
      bodyAppleAlbumId ??
      null;

    console.log(`✓ Album found: ID=${album.id}, Apple Music ID=${appleAlbumId ?? 'none'}`);

    if (!appleAlbumId) {
      console.log('❌ ERROR: Album has no Apple Music ID (credits or request)');
      return NextResponse.json(
        { success: false, error: 'Album has no Apple Music ID (credits or request)' },
        { status: 400 }
      );
    }
    ...
    console.log(`🔍 Fetching Apple Music tracks for album ${appleAlbumId}...`);

    const appleTracks = await fetchAppleAlbumTracks(appleAlbumId);
