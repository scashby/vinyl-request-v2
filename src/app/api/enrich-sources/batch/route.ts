// src/app/api/enrich-sources/batch/route.ts - MOVED from enrich-multi-batch + UPDATED API path
...
async function fetchAppleMusicLyrics(albumId: number): Promise<boolean> {
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/enrich-sources/apple-lyrics`, {
      method: 'POST',
