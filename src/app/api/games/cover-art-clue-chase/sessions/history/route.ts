...
export async function GET(request: NextRequest) {
  ...
  const [{ data: teams }, { data: scores }] = await Promise.all([
    sessionIds.length
      ? db.from("cacc_session_teams").select("session_id").in("session_id", sessionIds)
  ...
  const eventIds = Array.from(
    new Set(rows.map((row) => row.event_id).filter((value): value is number => Number.isFinite(value)))
  );
