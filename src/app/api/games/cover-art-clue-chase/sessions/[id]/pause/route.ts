...
export async function POST(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: session, error } = await db.from("cacc_sessions").select("id, status").eq("id", sessionId).maybeSingle();
  ...
  const { error: updateError } = await db.from("cacc_sessions").update({ status: "paused" }).eq("id", sessionId);
