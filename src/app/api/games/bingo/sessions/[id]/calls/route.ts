...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await db
    ...
    .select("id, session_id, playlist_track_key, call_index, ball_number, column_letter, track_title, artist_name, album_name, side, position, link_group, metadata_locked, metadata_synced_at, reveal_context, status, prep_started_at, called_at, completed_at, created_at")
