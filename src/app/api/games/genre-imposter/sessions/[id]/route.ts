...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: event }, { data: teams }, { data: rounds }, { data: calls }, { data: scores }] = await Promise.all([
    session.event_id
      ? db.from("events").select("id, title, date, time, location").eq("id", session.event_id).maybeSingle()
      : Promise.resolve({ data: null }),
    db.from("gi_session_teams").select("id").eq("session_id", sessionId),
    db.from("gi_session_rounds").select("id").eq("session_id", sessionId),
    db.from("gi_session_calls").select("id").eq("session_id", sessionId),
    db.from("gi_team_scores").select("id").eq("session_id", sessionId),
  ]);
