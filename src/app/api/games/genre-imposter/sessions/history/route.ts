...
export async function GET(request: NextRequest) {
  ...
  let query = db
    .from("gi_sessions")
    .select("id, event_id, session_code, title, status, current_round, current_call_index, created_at")
  ...
  const [{ data: teams }, { data: rounds }, { data: picks }] = await Promise.all([
    sessionIds.length
      ? db.from("gi_session_teams").select("session_id").in("session_id", sessionIds)
      : Promise.resolve({ data: [] as Array<{ session_id: number }> }),
    sessionIds.length
      ? db.from("gi_session_rounds").select("session_id").in("session_id", sessionIds)
      : Promise.resolve({ data: [] as Array<{ session_id: number }> }),
    sessionIds.length
      ? db.from("gi_round_team_picks").select("session_id").in("session_id", sessionIds)
      : Promise.resolve({ data: [] as Array<{ session_id: number }> }),
  ]);
