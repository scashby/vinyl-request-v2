...
async function insertEvent(sessionId: number, eventType: string, payload: Record<string, unknown>) {
  ...
  const { error } = await dbAny.from("ntt_session_events").insert({
...
async function getCallById(db: ReturnType<typeof getNameThatTuneDb>, sessionId: number, callId: number): Promise<CallRow | null> {
...
async function getQueueCalls(db: ReturnType<typeof getNameThatTuneDb>, sessionId: number, currentCallIndex: number): Promise<CallRow[]> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const dbAny = db as any;
