...
export async function POST(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ error: updateCallError }, { error: roundOpenError }, { error: closePreviousRoundsError }] = await Promise.all([
    db
      .from("lgr_session_calls")
      .update({
        status: "asked",
  ...
  if (roundOpenError) return NextResponse.json({ error: roundOpenError.message }, { status: 500 });
