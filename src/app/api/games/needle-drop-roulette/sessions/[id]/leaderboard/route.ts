...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: teams, error: teamsError }, { data: scores, error: scoresError }] = await Promise.all([
    db.from("ndr_session_teams").select("id, team_name, table_label, active").eq("session_id", sessionId).order("id", { ascending: true }),
    db
      .from("ndr_team_scores")
  ...
  const totals = new Map<
    number,
    { points: number; artistHits: number; titleHits: number; perfectCalls: number; scoredCalls: Set<number> }
  >();
