...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = awards.map((award) => {
    ...
    return {
      ...
      awarded_points: Number.isFinite(providedPoints) ? clampAwardedPoints(providedPoints) : defaultPoints,
  ...
  const { error: upsertError } = await db.from("ndr_team_scores").upsert(rows, { onConflict: "session_id,team_id,call_id" });
