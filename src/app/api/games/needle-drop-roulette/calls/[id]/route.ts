...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const patch: NdrCallPatch = { status: body.status };
  ...
  const { error: patchError } = await db.from("ndr_session_calls").update(patch).eq("id", callId);
  if (patchError) return NextResponse.json({ error: patchError.message }, { status: 500 });

  if (body.status === "asked") {
    const { data: session } = await db.from("ndr_sessions").select("id, started_at").eq("id", typedCall.session_id).maybeSingle();
