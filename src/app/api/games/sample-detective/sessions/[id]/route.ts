...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await db.from("sd_sessions").select("*").eq("id", sessionId).maybeSingle();
  ...
  const [{ data: event }, { data: playlist }, { data: calls }, { data: overlayEvent }] = await Promise.all([
    ...
    session.playlist_id
      ...
      : Promise.resolve({ data: null }),
    db.from("sd_session_calls").select("id").eq("session_id", sessionId),
    db
      .from("sd_session_events")
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const patch = Object.fromEntries(Object.entries(body).filter(([key]) => allowedFields.has(key))) as SdSessionPatch;
  ...
  if (hasAnyTimerChange && !hasExplicitTargetGap) {
    const { data: current } = await getSampleDetectiveDb()
      .from("sd_sessions")
  ...
  const db = getSampleDetectiveDb();
  const { error } = await db.from("sd_sessions").update(patch).eq("id", sessionId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
