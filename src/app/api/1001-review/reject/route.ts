// src/app/api/1001-review/reject/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { supabaseAdmin } from 'src/lib/supabaseAdmin';

type Body = {
  collection_id: number;
  album_1001_id?: number | null;
...
function parseBody(x: unknown): Body | null {
  ...
  const album_1001_id = x.album_1001_id == null ? null : Number(x.album_1001_id);
  const confidence = typeof x.confidence === 'number' ? x.confidence : null;
  const notes = typeof x.notes === 'string' ? x.notes : null;
  if (!Number.isFinite(collection_id)) return null;
