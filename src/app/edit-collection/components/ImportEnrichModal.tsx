...
import { supabase } from 'lib/supabaseClient'; // Fixed: Use singleton & alias
import ConflictResolutionModal from './ConflictResolutionModal';
import { type FieldConflict } from 'lib/conflictDetection'; // Fixed: Use alias

// --- 1. DATA CATEGORY DEFINITIONS ---
export type DataCategory = 
  | 'artwork' | 'credits' | 'tracklists' | 'audio_analysis' 
  | 'genres' | 'streaming_links' | 'reviews' | 'chart_data' | 'release_metadata';

const DATA_CATEGORY_LABELS: Record<DataCategory, string> = {
  artwork: 'Artwork & Images',
  credits: 'Credits & Personnel',
  tracklists: 'Tracklists & Durations',
  audio_analysis: 'Audio Analysis (BPM/Key)',
  genres: 'Genres & Styles',
  streaming_links: 'Streaming IDs & URLs',
  reviews: 'Reviews & Ratings',
  chart_data: 'Charts & Popularity',
  release_metadata: 'Release Metadata',
};

const DATA_CATEGORY_DESCRIPTIONS: Record<DataCategory, string> = {
  artwork: 'Front covers, back covers, spine images, and inner sleeves.',
  credits: 'Musicians, producers, engineers, and songwriters.',
  tracklists: 'Complete track listings, disk sides, and track durations.',
  audio_analysis: 'Tempo (BPM), musical key, danceability, and energy metrics.',
  genres: 'Primary genres, sub-styles, and mood tags.',
  streaming_links: 'Links to Spotify, Apple Music, and Last.fm.',
  reviews: 'Critic reviews, user ratings, and editorial notes.',
  chart_data: 'Billboard chart positions and sales certifications.',
  release_metadata: 'Catalog numbers, barcodes, record labels, and release dates.',
};

const DATA_CATEGORY_ICONS: Record<DataCategory, string> = {
  artwork: '🖼️', credits: '👥', tracklists: '📝', audio_analysis: '📊',
  genres: '🏷️', streaming_links: '🔗', reviews: '⭐', chart_data: '📈', release_metadata: '💿',
};
...
// Type definition for the API response items
...
export default function ImportEnrichModal({ isOpen, onClose, onImportComplete }: ImportEnrichModalProps) {
  ...
  // Removed local createClient() call; using imported singleton 'supabase'
  ...
  function getServicesForSelection() {
    ...
    if (selectedCategories.has('artwork')) { s.coverArt = true; s.spotify = true; s.discogs = true; s.appleMusicEnhanced = true; }
    if (selectedCategories.has('credits')) { s.musicbrainz = true; s.discogs = true; s.wikipedia = true; }
    if (selectedCategories.has('tracklists')) { s.discogs = true; s.spotify = true; }
    if (selectedCategories.has('audio_analysis')) { s.spotify = true; }
    if (selectedCategories.has('genres')) { s.discogs = true; s.lastfm = true; s.spotify = true; }
    if (selectedCategories.has('streaming_links')) { s.spotify = true; s.appleMusicEnhanced = true; s.lastfm = true; }
    if (selectedCategories.has('reviews') || selectedCategories.has('release_metadata')) { s.discogs = true; s.musicbrainz = true; }
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
      <div style={{ backgroundColor: 'white', borderRadius: '8px', width: '1200px', maxWidth: '95vw', maxHeight: '95vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f59e0b', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '600' }}>⚡ Collection Data Enrichment</h2>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ marginBottom: '20px', padding: '16px', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '6px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '8px' }}>Select Data to Enrich</h3>
              ...
              <div style={{ marginBottom: '16px', padding: '16px', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '6px', display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <select value={folderFilter} onChange={(e) => setFolderFilter(e.target.value)} disabled={enriching} style={{ padding: '6px', borderRadius: '4px' }}>
                    <option value="">All Folders</option>
                    {folders.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <label style={{ fontWeight: '600', fontSize: '14px' }}>Batch Size:</label>
                  <select value={batchSize} onChange={(e) => setBatchSize(e.target.value)} disabled={enriching} style={{ padding: '6px', borderRadius: '4px' }}>
        ...
        <div style={{ padding: '16px 20px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
          <button onClick={onClose} disabled={enriching} style={{ padding: '8px 16px', backgroundColor: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: '4px' }}>Close</button>
          <button 
      ...
      {showCategoryModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 40000 }}>
          <div style={{ backgroundColor: 'white', borderRadius: '8px', width: '1000px', maxWidth: '90vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px' }}>
                    {categoryAlbums.map(album => (
                      <div key={album.id} style={{ border: '1px solid #e5e7eb', borderRadius: '6px', padding: '8px' }}>
                        ...
                        <div style={{ fontWeight: '600', fontSize: '11px', whiteSpace: 'nowrap', overflow: 'hidden' }}>{album.title}</div>
...
function DataCategoryCard({ category, count, subcounts, selected, onToggle, disabled }: { category: DataCategory; count: number; subcounts?: { label: string; count: number }[]; selected: boolean; onToggle: () => void; disabled: boolean; }) {
  ...
  return (
    <div style={{ border: `2px solid ${selected ? '#f59e0b' : '#e5e7eb'}`, borderRadius: '6px', padding: '12px', backgroundColor: selected ? '#fff7ed' : 'white', cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.6 : 1 }}>
      <div onClick={disabled ? undefined : onToggle} style={{ display: 'flex', gap: '10px' }}>
        <input type="checkbox" checked={selected} onChange={onToggle} disabled={disabled} style={{ marginTop: '2px' }} />
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
            <span>{DATA_CATEGORY_ICONS[category]}</span>
            <span style={{ fontWeight: '600', fontSize: '14px' }}>{DATA_CATEGORY_LABELS[category]}</span>
            <span style={{ marginLeft: 'auto', fontWeight: '700', color: count > 0 ? '#f59e0b' : '#10b981' }}>{count.toLocaleString()}</span>
          </div>
          <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>{DATA_CATEGORY_DESCRIPTIONS[category]}</div>
          {subcounts && subcounts.length > 0 && (
            <button onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }} style={{ background: 'none', border: 'none', color: '#3b82f6', fontSize: '11px', cursor: 'pointer' }}>
          ...
          {expanded && subcounts && (
            <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #e5e7eb', fontSize: '11px' }}>
              {subcounts.map((sub, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between' }}><span>{sub.label}:</span><b>{sub.count}</b></div>
              ))}
            </div>
          )}
        </div>
