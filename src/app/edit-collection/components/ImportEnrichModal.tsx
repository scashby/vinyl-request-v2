...
export default function ImportEnrichModal({ isOpen, onClose, onImportComplete }: ImportEnrichModalProps) {
  ...
  async function processBatchAndSave(results: CandidateResult[]) {
    ...
    if (autoUpdates.length > 0) {
      await Promise.all(autoUpdates.map(u => supabase.from('v2_legacy_archive').update(u.fields).eq('id', u.id)));
    }
    ...
    if (untouchedIds.length > 0) {
      ...
      await supabase.from('v2_legacy_archive')
  ...
  async function handleSkip(albumId: number) {
    const timestamp = new Date().toISOString();
    await supabase.from('v2_legacy_archive').update({ last_reviewed_at: timestamp }).eq('id', albumId);
  ...
  async function handleSingleAlbumSave(
    ...
    if (Object.keys(updates).length > 0) {
        ...
        const { error } = await supabase.from('v2_legacy_archive').update(updates).eq('id', albumId);
        if (error) {
