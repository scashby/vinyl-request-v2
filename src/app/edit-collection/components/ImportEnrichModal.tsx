...
const toJsonValue = (value: unknown): Json | null => {
...
export default function ImportEnrichModal({ isOpen, onClose, onImportComplete }: ImportEnrichModalProps) {
  ...
  async function processBatchAndSave(results: CandidateResult[]) {
    ...
    results.forEach((item) => {
          ...
          if (isCurrentEmpty && (isMerge || singleValuesAgree)) {
              ...
              if (!areValuesEqual(currentVal, proposedValue)) {
                  ...
                  newConflicts.push({
                      ...
                      catalog_number: (album.cat_no as string) || '',
      ...
      // Keep conflict scan state deterministic even when no fields were changed.
      const hasConflicts = newConflicts.some(c => c.album_id === album.id);
      if (!hasConflicts) {
          updatesForAlbum.enrichment_summary = 'No conflicts found';
      }
    ...
    if (historyUpdates.length > 0) {
       const resolutionRows: ResolutionInsert[] = historyUpdates.map((item) => ({
         album_id: item.album_id,
         field_name: item.field_name,
         source: item.source,
         resolution: item.resolution,
         kept_value: toJsonValue(item.kept_value),
         resolved_at: item.resolved_at,
       }));
       await supabase
         .from('import_conflict_resolutions')
         .upsert(resolutionRows, { onConflict: 'album_id,field_name,source' });
    }
    ...
    // FINAL SAFETY: mark untouched IDs with a summary note so they do not keep cycling.
    ...
    if (untouchedIds.length > 0) {
      await supabase.from('inventory')
        .update({ personal_notes: 'No enrichment changes found in latest scan.' })
        .in('id', untouchedIds);
    }
  ...
  async function handleSkip(albumId: number) {
    await supabase
      .from('inventory')
      .update({ personal_notes: 'Skipped in enrichment review.' })
      .eq('id', albumId);
  ...
  async function handleSingleAlbumSave(
    ...
    const resolutionRecords: ResolutionInsert[] = [];
    ...
    if (Object.keys(updates).length > 0) {
        ...
        await supabase
          .from('inventory')
          .update({ personal_notes: 'No direct field updates selected during review.' })
          .eq('id', albumId);
    }
