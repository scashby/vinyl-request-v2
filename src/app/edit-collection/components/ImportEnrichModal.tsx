...
const ALLOWED_COLUMNS = new Set([
  ...
  'image_url', 'back_image_url', 'sell_price', 'media_condition', 'location', 
  ...
  'lastfm_tags', 'personal_notes', 'release_notes', 'enriched_metadata', 'enrichment_summary', 'genres', 'styles', 'original_release_date',
    ...
    while ((collectedConflicts.length < targetConflicts || specificAlbumIds) && hasMoreRef.current) {
      ...
      try {
        const payload = {
          ...
          location: folderFilter || undefined, // Mapped to location in API
    ...
    results.forEach((item) => {
      ...
      for (const source of GLOBAL_PRIORITY) {
         ...
         Object.entries(sourceData).forEach(([key, value]) => {
            ...
            // *** REDIRECT 'NOTES' TO PERSONAL_NOTES ***
            if (key === 'notes' && value) {
               // We map incoming notes to personal_notes if appropriate, or check schema
               if (!fieldCandidates['personal_notes']) fieldCandidates['personal_notes'] = {};
               fieldCandidates['personal_notes'][source] = value;
               return; 
            }
...
{
type: uploaded file
fileName: scashby/vinyl-request-v2/vinyl-request-v2-cad997bdc04ac4bc8d7de5d65f383d1bde928b2a/src/app/api/enrich-sources/fetch-candidates/route.ts
fullContent:
