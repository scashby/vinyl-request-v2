...
export default function ManagePickListsModal({ isOpen, onClose }: ManagePickListsModalProps) {
  ...
  if (!selectedList) {
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 30001 }} onClick={onClose}>
        <div style={{ backgroundColor: 'white', borderRadius: '6px', width: '500px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ padding: '16px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f9fafb' }}>
            <p style={{ margin: 0, fontSize: '13px', color: '#6b7280' }}>Select a list below to view and manage its items. You can edit names, merge duplicates, and delete unused entries.</p>
          </div>
          <div style={{ padding: '16px' }}>
            <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500', color: '#374151' }}>Select a list</label>
            <select value="" onChange={(e) => setSelectedList(e.target.value)} style={{ width: '100%', padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '13px', outline: 'none', backgroundColor: 'white', cursor: 'pointer', color: '#111827' }}>
              <option value="">Choose a list...</option>
              {Object.entries(PICK_LIST_CONFIGS).sort((a, b) => a[1].label.localeCompare(b[1].label)).map(([key, config]) => (
  ...
  return (
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 30001 }} onClick={() => setSelectedList(null)}>
        <div style={{ backgroundColor: 'white', borderRadius: '6px', width: mergeMode ? '900px' : '600px', maxHeight: '700px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }} onClick={(e) => e.stopPropagation()}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f97316' }}>
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600', color: 'white' }}>Manage {config.label}s</h3>
            <button onClick={() => setSelectedList(null)} style={{ background: 'transparent', border: 'none', color: 'white', fontSize: '20px', cursor: 'pointer', padding: '0 4px', lineHeight: '1' }}>×</button>
          </div>
          ...
          {mergeMode && (
            <div style={{ padding: '10px 16px', backgroundColor: '#fef3c7', borderBottom: '1px solid #fbbf24', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {selectedItems.size >= 2 && (
                <button onClick={handleMerge} style={{ padding: '5px 14px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '4px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Merge Selected</button>
              )}
            </div>
          )}

          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            <div style={{ flex: mergeMode ? '0 0 60%' : '1', overflowY: 'auto', padding: '8px 16px' }}>
              {items.map((item) => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 8px', borderRadius: '3px', backgroundColor: selectedItems.has(item.id) ? '#eff6ff' : 'transparent', marginBottom: '1px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1 }}>
                    ...
                    <span style={{ fontSize: '13px', color: '#111827' }}>{item.name} {item.count !== undefined && `(${item.count})`}</span>
                  </div>
                  {!mergeMode && (
            ...
            {mergeMode && previewItems.length > 0 && (
              <div style={{ flex: '0 0 40%', padding: '16px', backgroundColor: '#f9fafb', overflowY: 'auto', borderLeft: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#374151', marginBottom: '12px' }}>Selected for merge:</div>
                {previewItems.map(item => (
                  <div key={item.id} style={{ padding: '6px 8px', backgroundColor: 'white', borderRadius: '3px', marginBottom: '4px', fontSize: '13px', display: 'flex', justifyContent: 'space-between', color: '#111827' }}>
                    <span>{item.name}</span>
          ...
          <div style={{ padding: '12px 16px', borderTop: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between' }}>
            <button onClick={() => setSelectedList(null)} style={{ padding: '6px 20px', background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '4px', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>Back to List Selection</button>
            <button onClick={() => { setMergeMode(!mergeMode); setSelectedItems(new Set()); }} style={{ padding: '6px 20px', background: mergeMode ? '#ef4444' : '#6b7280', color: 'white', border: 'none', borderRadius: '4px', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>{mergeMode ? 'Cancel Merge' : 'Merge Mode'}</button>
          </div>
        </div>
      </div>
