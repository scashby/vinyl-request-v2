...
export default function BingoHistoryPage() {
  ...
  const load = async () => {
    const res = await fetch("/api/games/bingo/sessions/history");
    if (!res.ok) return;
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#141414,#0a0a0a)] p-6 text-stone-100">
      <div className="mx-auto max-w-5xl rounded-3xl border border-stone-700 bg-black/50 p-6">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-3xl font-black uppercase text-amber-200">Bingo History</h1>
          ...
          <div className="space-y-2 text-sm">
            {rows.map((row) => (
              <div key={row.id} className="rounded border border-stone-700 bg-stone-950/70 p-3">
                <p>{new Date(row.created_at).toLocaleString()} · {row.session_code} · {row.playlist_name}</p>
                <p className="text-stone-400">Calls: {row.calls_played} · Status: {row.status}</p>
              </div>
            ))}
          </div>
        )}
