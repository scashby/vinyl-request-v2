...
export default function LyricGapRelaySetupPage() {
  ...
  const createSession = async () => {
    ...
    try {
      ...
      window.open(`/admin/games/lyric-gap-relay/host?sessionId=${payload.id}`, "_blank", "noopener,noreferrer");
    } catch (error) {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_20%,#3a1b2f,transparent_45%),linear-gradient(180deg,#111,#070707)] p-6 text-stone-100">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-3xl border border-fuchsia-900/40 bg-black/45 p-6">
          ...
          <button disabled={!playlistId || playlistTooSmall || !preflightComplete || teamNames.length < 2 || roundCountWarning || creating} onClick={createSession} className="mt-5 rounded bg-fuchsia-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">
