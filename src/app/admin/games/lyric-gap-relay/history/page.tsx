...
export default function LyricGapRelayHistoryPage() {
  ...
  const load = async (eventFilter: number | null) => {
    const [eventRes, historyRes] = await Promise.all([
      ...
      fetch(`/api/games/lyric-gap-relay/sessions/history${eventFilter ? `?eventId=${eventFilter}` : ""}`),
    ]);
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#141414,#0a0a0a)] p-6 text-stone-100">
      <div className="mx-auto max-w-5xl rounded-3xl border border-stone-700 bg-black/50 p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-3xl font-black uppercase text-fuchsia-200">Lyric Gap Relay History</h1>
          <div className="flex items-center gap-2">
            <Link href="/admin/games/lyric-gap-relay" className="rounded border border-stone-700 px-3 py-1 text-sm">Back to Setup</Link>
