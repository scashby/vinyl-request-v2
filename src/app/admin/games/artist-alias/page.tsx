...
export default function ArtistAliasSetupPage() {
  ...
  const createSession = async () => {
    ...
    try {
      ...
      window.open(`/admin/games/artist-alias/host?sessionId=${payload.id}`, "_blank", "noopener,noreferrer");
    } catch (error) {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_20%,#25203a,transparent_45%),linear-gradient(180deg,#111,#060606)] p-6 text-stone-100">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl border border-violet-900/40 bg-black/45 p-6">
          ...
          <p className="mt-2 text-sm text-stone-300">Artist Alias reveals clues about one artist in stages, lets teams lock in early for bigger points, and the team with the most points wins.</p>
        ...
        <section className="rounded-3xl border border-violet-900/40 bg-black/45 p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button className="rounded bg-violet-500 px-4 py-2 font-semibold text-black disabled:opacity-40" disabled={!playlistId || playlistTooSmall || creating || !preflightComplete || roundCountWarning} onClick={createSession}>
