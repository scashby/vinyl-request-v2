...
export default function CoverArtClueChaseEditPage() {
  ...
  const [finalRevealPoints, setFinalRevealPoints] = useState(5);
  ...
  useEffect(() => {
    ...
    fetch(`/api/games/cover-art-clue-chase/sessions/${sessionId}`)
      ...
      .then((data: Session) => {
        ...
        setStageOnePoints(data.stage_one_points || 3);
        setStageTwoPoints(data.stage_two_points || 2);
        setFinalRevealPoints(data.final_reveal_points || 5);
        setShowLogo(data.show_logo ?? true);
        setWelcomeHeading(data.welcome_heading_text ?? "Welcome to Cover Art Clue Chase");
        setWelcomeMessage(data.welcome_message_text ?? "Study each image and identify the record.");
        setIntermissionHeading(data.intermission_heading_text ?? "Intermission");
        setIntermissionMessage(data.intermission_message_text ?? "Take a breath. Next round starts soon.");
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/games/cover-art-clue-chase/sessions/${sessionId}`, {
        method: "PATCH",
      ...
      router.push("/admin/games/cover-art-clue-chase");
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-stone-900 p-6 text-white">
      <h1 className="mb-6 text-2xl font-black uppercase text-teal-300">Edit Session — Cover Art Clue Chase</h1>

      <div className="space-y-6 max-w-xl">
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-teal-400">Session Info</h2>
          ...
          <label className="block text-sm">
             Clues ({roundCount})
            <input className="mt-1 w-full" type="range" min={8} max={14}
          ...
          {[
            ...
            <label key={label} className="block text-sm">
              ...
              <input className="mt-1 w-full rounded border border-stone-700 bg-stone-950 px-3 py-2 text-white"
                ...
                value={val} onChange={(e) => setter(Math.min(max, Math.max(0, Number(e.target.value) || 0)))} />
            </label>
          ))}
        </section>

        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-3">
          <h2 className="text-sm font-bold uppercase text-teal-400">Display</h2>
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} disabled={saving}
            className="flex-1 rounded bg-teal-700 px-4 py-2 font-bold text-white hover:bg-teal-600 disabled:opacity-50">
            {saving ? "Saving..." : "Save"}
          </button>
