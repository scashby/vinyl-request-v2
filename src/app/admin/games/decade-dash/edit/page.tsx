...
export default function DecadeDashEditPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/games/decade-dash/sessions/${sessionId}`)
      ...
      .then((data: Session) => {
        setSession(data);
        setTitle(data.title);
        setSongCount(data.song_count || 10);
        setShowLogo(data.show_logo ?? true);
        setWelcomeHeading(data.welcome_heading_text ?? "Welcome to Decade Dash");
        setWelcomeMessage(data.welcome_message_text ?? "Identify songs from a specific era.");
        setIntermissionHeading(data.intermission_heading_text ?? "Intermission");
        setIntermissionMessage(data.intermission_message_text ?? "Taking a brief break.");
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/games/decade-dash/sessions/${sessionId}`, {
        method: "PATCH",
      ...
      router.push("/admin/games/decade-dash");
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-stone-900 p-6 text-white">
      <h1 className="mb-6 text-2xl font-black uppercase text-pink-300">Edit Session — Decade Dash</h1>

      <div className="space-y-6 max-w-xl">
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-pink-400">Session Info</h2>
          ...
          <label className="block text-sm">
            Songs in Playlist (5-20)
            <input className="mt-1 w-full rounded border border-stone-700 bg-stone-950 px-3 py-2 text-white"
              type="number" min={5} max={20}
              value={songCount} onChange={(e) => setSongCount(Math.min(20, Math.max(5, Number(e.target.value) || 10)))} />
          </label>
        </section>

        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-3">
          <h2 className="text-sm font-bold uppercase text-pink-400">Display</h2>
        ...
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-pink-400">Overlay Text</h2>
          {[
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} disabled={saving}
            className="flex-1 rounded bg-pink-700 px-4 py-2 font-bold text-white hover:bg-pink-600 disabled:opacity-50">
            {saving ? "Saving..." : "Save"}
          </button>
          <button onClick={() => router.back()}
            className="flex-1 roundedborder border-stone-600 px-4 py-2 hover:bg-stone-800">
            Cancel
          </button>
        </div>
      </div>
