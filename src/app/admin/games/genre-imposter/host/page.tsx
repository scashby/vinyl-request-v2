...
export default function GenreImposterHostPage() {
  ...
  const [swapTargetCall, setSwapTargetCall] = useState<Session["calls"][number] | null>(null);
  ...
  const patchCallMetadata = useCallback(
    async (callId: number, patch: Record<string, unknown>) => {
      const res = await fetch(`/api/games/genre-imposter/calls/${callId}`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        ...
        throw new Error(payload?.error ?? "Failed to save round metadata");
      }
      await load();
    },
    [load]
  ...
  const setOverlay = async (mode: "none" | "welcome" | "countdown" | "intermission" | "thanks", durationSeconds?: number) => {
    setOverlayBusy(true);
    try {
      const res = await fetch(`/api/games/genre-imposter/sessions/${sessionId}/overlay`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const runSessionSwap = useCallback(async (candidate: InventorySearchCandidate) => {
    ...
    setSwapMessage(null);
    try {
      const response = await fetch(`/api/games/genre-imposter/sessions/${sessionId}/swap-track`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
                ...
                <tbody>
                  {(session?.calls ?? []).map((call) => (
                    <tr key={call.id} className="border-t border-stone-800 align-top">
                      ...
                      <td className="py-1">
                        <InlineEditableCell
                          onSave={(nextValue) => patchCallMetadata(call.id, { artist: nextValue || null })}
                          value={call.artist ?? ""}
                        />
                      </td>
                      <td className="py-1">
                        <InlineEditableCell
                          onSave={(nextValue) => patchCallMetadata(call.id, { title: nextValue || null })}
                          value={call.title ?? ""}
                        />
                      </td>
                      <td className="py-1">
            ...
            <div className="rounded-2xl border border-stone-700 bg-black/45 p-4">
              ...
              <div className="mt-3 rounded border border-stone-700 bg-stone-950/60 p-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  <button disabled={overlayBusy} onClick={() => setOverlay("welcome")} className="rounded border border-emerald-700 px-2 py-1 disabled:opacity-50">Welcome</button>
                  <button disabled={overlayBusy} onClick={() => setOverlay("countdown", 300)} className="rounded border border-emerald-700 px-2 py-1 disabled:opacity-50">Countdown 5m</button>
                  <button disabled={overlayBusy} onClick={toggleIntermission} className="rounded border border-emerald-700 px-2 py-1 disabled:opacity-50">Intermission</button>
                  <button disabled={overlayBusy} onClick={() => setOverlay("thanks")} className="rounded border border-lime-700 px-2 py-1 disabled:opacity-50">Thanks</button>
            ...
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-stone-200">
                  Replacing: <span className="font-semibold">{swapTargetCall.title ?? "(untitled)"}</span> - {swapTargetCall.artist ?? "(unknown artist)"}
                </p>
              </div>
              <button
