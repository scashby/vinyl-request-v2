...
export default function GenreImposterSetupPage() {
  ...
  const createSession = async () => {
    ...
    try {
      ...
      window.open(`/admin/games/genre-imposter/host?sessionId=${payload.id}`, "_blank", "noopener,noreferrer");
    } catch (error) {
  ...
  const roundsStatus = useMemo(() => {
    if (!playlistId) {
      return {
        ...
        message: "No playlist selected yet. Leave this blank for auto-generated rounds after selecting a playlist bank, or paste manual rounds now.",
      };
    }

    if (usingAutoGeneratedRounds) {
      return {
        className: "text-emerald-300",
        message: `No manual rounds provided. ${roundCount} rounds will be auto-generated from the selected playlist bank.`,
      };
    }

    if (roundCountWarning) {
      return {
        className: "text-amber-300",
        message: `Valid rounds: ${rounds.length}. Minimum required for current rounds: ${roundCount}. Format: Category | A~Song~Source | B~Song~Source | C~Song~Source | ImposterIndex(1-3) | Reason key.`,
      };
    }

    return {
      className: "text-stone-400",
      message: `Valid rounds: ${rounds.length}. Format: Category | A~Song~Source | B~Song~Source | C~Song~Source | ImposterIndex(1-3) | Reason key.`,
    };
  }, [playlistId, roundCount, roundCountWarning, rounds.length, usingAutoGeneratedRounds]);
        ...
        <section className="rounded-3xl border border-emerald-900/40 bg-black/45 p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm">Playlist Pull Rounds (one per line, optional) <InlineFieldHelp label="Playlist Pull Rounds" />
              <textarea
                className="mt-1 h-40 w-full rounded border border-stone-700 bg-stone-950 px-3 py-2"
                value={roundDeckText}
                onChange={(e) => setRoundDeckText(e.target.value)}
                placeholder="Category | Artist~Song~Source | Artist~Song~Source | Artist~Song~Source | 3 | Reason key"
              />
              <p className={`mt-1 text-xs ${roundsStatus.className}`}>{roundsStatus.message}</p>
            </label>
          </div>
              ...
              {sessions.map((session) => (
                <div key={session.id} className="rounded-xl border border-stone-700 bg-stone-950/70 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    ...
                    <button className="rounded border border-stone-600 px-2 py-1" onClick={() => window.open("/admin/games/genre-imposter/history", "_blank", "noopener,noreferrer")}>History</button>
                  </div>
                </div>
              ))}
            </div>
