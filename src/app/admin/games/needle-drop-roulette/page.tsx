...
export default function NeedleDropRouletteSetupPage() {
  ...
  const createSession = async () => {
    ...
    try {
      ...
      window.open(`/admin/games/needle-drop-roulette/host?sessionId=${payload.id}`, "_blank", "noopener,noreferrer");
    } catch (error) {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_20%,#4a1707,transparent_45%),linear-gradient(180deg,#111,#070707)] p-6 text-stone-100">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl border border-orange-900/40 bg-black/45 p-6">
          ...
          <p className="mt-2 text-sm text-stone-300">Needle Drop Roulette drops the needle at random, plays a short clip, and has teams guess the artist and song before the reveal; the team with the most correct guesses wins.</p>
              ...
              {sessions.map((session) => (
                <div key={session.id} className="rounded-xl border border-stone-700 bg-stone-950/70 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    ...
                    <button className="rounded border border-stone-600 px-2 py-1" onClick={() => window.open("/admin/games/needle-drop-roulette/history", "_blank", "noopener,noreferrer")}>History</button>
                  </div>
                </div>
              ))}
            </div>
