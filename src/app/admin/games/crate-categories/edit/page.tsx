...
export default function CrateCategoriesEditPage() {
  ...
  const [categoryPoints, setCategoryPoints] = useState(3);
  ...
  useEffect(() => {
    ...
    fetch(`/api/games/crate-categories/sessions/${sessionId}`)
      ...
      .then((data: Session) => {
        ...
        setCategoryPoints(data.category_points || 3);
        setShowLogo(data.show_logo ?? true);
        setWelcomeHeading(data.welcome_heading_text ?? "Welcome to Crate Categories");
        setWelcomeMessage(data.welcome_message_text ?? "Guess what connects these tracks in the category.");
        setIntermissionHeading(data.intermission_heading_text ?? "Intermission");
        setIntermissionMessage(data.intermission_message_text ?? "Pause and prepare for the next category.");
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/games/crate-categories/sessions/${sessionId}`, {
        method: "PATCH",
      ...
      router.push("/admin/games/crate-categories");
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-stone-900 p-6 text-white">
      <h1 className="mb-6 text-2xl font-black uppercase text-purple-300">Edit Session — Crate Categories</h1>

      <div className="space-y-6 max-w-xl">
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-purple-400">Session Info</h2>
        ...
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-purple-400">Scoring</h2>
          <label className="block text-sm">
            Points Per Correct (1-5)
            <input className="mt-1 w-full rounded border border-stone-700 bg-stone-950 px-3 py-2 text-white"
              type="number" min={1} max={5}
              value={categoryPoints} onChange={(e) => setCategoryPoints(Math.min(5, Math.max(1, Number(e.target.value) || 1)))} />
          </label>
        </section>

        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-3">
          <h2 className="text-sm font-bold uppercase text-purple-400">Display</h2>
        ...
        <section className="rounded-2xl border border-stone-700 bg-black/40 p-4 space-y-4">
          <h2 className="text-sm font-bold uppercase text-purple-400">Overlay Text</h2>
          {([
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} disabled={saving}
            className="flex-1 rounded bg-purple-700 px-4 py-2 font-bold text-white hover:bg-purple-600 disabled:opacity-50">
            {saving ? "Saving..." : "Save"}
          </button>
