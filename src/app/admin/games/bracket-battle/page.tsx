...
export default function BracketBattleSetupPage() {
  ...
  const minimumPlaylistTracks = useMemo(
    () => Math.max(bracketSize + 2, entries.length),
    [bracketSize, entries.length]
  );
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_20%,#1f2f50,transparent_45%),linear-gradient(180deg,#111,#070707)] p-6 text-stone-100">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-3xl border border-blue-900/40 bg-black/45 p-6">
          ...
          <p className="mt-2 text-sm text-stone-300">Bracket Battle is a song tournament where tracks face off head-to-head, teams pick each winner, and the team with the best bracket score wins.</p>
        ...
        <section className="rounded-3xl border border-blue-900/40 bg-black/45 p-6">
          ...
          <button disabled={!playlistId || playlistTooSmall || !preflightComplete || teamNames.length < 2 || entryWarning || creating} onClick={createSession} className="mt-5 rounded bg-blue-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">
