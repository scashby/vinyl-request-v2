...
export default function BracketBattleHistoryPage() {
  ...
  const load = async (eventFilter: number | null) => {
    const [eventRes, historyRes] = await Promise.all([
      ...
      fetch(`/api/games/bracket-battle/sessions/history${eventFilter ? `?eventId=${eventFilter}` : ""}`),
    ]);
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#141414,#0a0a0a)] p-6 text-stone-100">
      <div className="mx-auto max-w-5xl rounded-3xl border border-stone-700 bg-black/50 p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-3xl font-black uppercase text-cyan-200">Bracket Battle History</h1>
          ...
          <div className="space-y-2 text-sm">
            {rows.map((row) => (
              <div key={row.id} className="rounded border border-stone-700 bg-stone-950/70 p-3">
                ...
                <p className="text-stone-400">Event: {row.event_title ?? "(none)"} · Teams: {row.teams} · Round: {row.current_round} · Matchup Index: {row.current_matchup_index} · Matchups: {row.matchups_total} · Picks: {row.picks_logged} · Status: {row.status}</p>
              </div>
            ))}
          </div>
        )}
