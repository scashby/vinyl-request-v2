// app/admin/featured-events/page.tsx
// Admin Featured Events page ("/admin/featured-events")
// Lets admins manage is_featured_grid and featured_priority
// for upcoming / TBA events. "Up Next" is now automatic and
// not manually controlled here.
...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      ...
      const { data, error } = await supabase
        .from("events")
        .select(
          "id,title,date,location,is_featured_grid,featured_priority"
        )
  ...
  async function updateEventFlags(id: number, updates: Partial<FeaturedEvent>) {
  ...
  function handleToggleGrid(eventRow: FeaturedEvent, nextChecked: boolean) {
    updateEventFlags(eventRow.id, { is_featured_grid: nextChecked });
  }

  function handlePriorityChange(eventRow: FeaturedEvent, value: string) {
  ...
  return (
    ...
    >
      ...
      <p style={{ marginBottom: "1rem", fontSize: "0.95rem", color: "#555" }}>
        Use this page to control which upcoming/TBA events appear in the{" "}
        <strong>Featured Grid</strong> and in what{" "}
        <strong>priority order</strong>. <br />
        The <strong>Up Next</strong> section is now automatic and simply shows
        the next upcoming events by date.
      </p>
                ...
                <tr key={ev.id}>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        handlePriorityChange(ev, e.target.value)
                      }
                    />
                  </td>
                </tr>
